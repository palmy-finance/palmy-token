 ...
 * @title TokenVesting
 */
contract TokenVesting is PalmyOwnable, ReentrancyGuard, Initializable {
   ...
   */
  constructor(address owner) PalmyOwnable(owner) {}

  function initialzie(address token_) external initializer {
  ...
  function createVestingSchedule(
    ...
    require(_beneficiary != address(0), 'TokenVesting: benefiticary must not be empty');
  ...
  function revoke(
    bytes32 vestingScheduleId
  ) public onlyOwner onlyIfVestingScheduleNotRevoked(vestingScheduleId) {
  ...
  function _computeReleasableAmount(
    ...
    if ((currentTime < vestingSchedule.cliff) || vestingSchedule.revoked == true) {
      ...
      uint256 vestedAmount = vestingSchedule.amountTotal.mul(vestedSeconds).div(
